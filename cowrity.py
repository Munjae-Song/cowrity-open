...
#import notion_client # Notion API를 사용하기 위한 패키지
...
if not all([CLAUDE_API_KEY, PERPLEXITY_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, 
            NOTION_API_KEY, NOTION_DATABASE_ID, GROQ_API_KEY]):
    missing_keys = []
...
class CowrityApp:
    def __init__(self, root):
        ...
        # 언어 모델 옵션
        model_options = ["Claude Sonnet 4(일반)","Claude Opus 4(정교함/비쌈)","Claude Haiku 3.5(단순,저가)","Claude Opus 4.1(비쌈)",
                         "Perplexity Sonar(일반)", "Perplexity Sonar Pro(정교함, 비쌈)", 
                         "Gemini 2.5 Flash(일반)", "Gemini 2.5 Pro(정교함, 비쌈)", 
                         "GPT-4.1(일반)", "OpenAI o3(추론모델)", 
                         "gpt-5(정밀, 비쌈)", "gpt-5-mini(중간)", "gpt-5-nano(빠름, 저가)",
                         "OpenAI-OSS-120B"]
        
        # 1차 언어모델 선택
        ttk.Label(model_frame, text="1차 언어모델:").grid(row=0, column=0, sticky=tk.W, padx=80)
        self.model1_var = tk.StringVar(value="gpt-5-mini(중간)")
        ...
        self.model2_var = tk.StringVar(value="Perplexity Sonar(일반)")
        ...
        try:
            ...
            if model_name == "Claude Sonnet 4(일반)":
                ...
                if not is_error:
                    self.root.after(0, lambda: self.copy_response_2input_btn.configure(state=tk.NORMAL))
                    self.root.after(0, lambda: self.notion_upload_btn.configure(state=tk.NORMAL))
            elif model_name == "Claude Opus 4.1(비쌈)":
                response = self.claude_api(prompt, task_type, model_name, selected_purpose)
                # 오류 응답인지 확인
                is_error = response.startswith("❌")
               ...
               #
                ...
                # 오류 응답인지 확인
                #"gpt-5(정밀, 비쌈)", "gpt-5-mini(중간)", "gpt-5-nano(빠름, 저가)",
            elif model_name == "gpt-5(정밀, 비쌈)":
                response = self.openai_api(prompt, task_type, model_name, selected_purpose)
                is_error = response.startswith("❌")
                if not is_error:
                    self.root.after(0, lambda: self.copy_response_2input_btn.configure(state=tk.NORMAL))
                    self.root.after(0, lambda: self.notion_upload_btn.configure(state=tk.NORMAL))
            elif model_name == "gpt-5-mini(중간)":
                response = self.openai_api(prompt, task_type, model_name, selected_purpose)
                is_error = response.startswith("❌")
                if not is_error:
                    self.root.after(0, lambda: self.copy_response_2input_btn.configure(state=tk.NORMAL))
                    self.root.after(0, lambda: self.notion_upload_btn.configure(state=tk.NORMAL))
            elif model_name == "gpt-5-nano(빠름, 저가)":
                response = self.openai_api(prompt, task_type, model_name, selected_purpose)
                is_error = response.startswith("❌")
                if not is_error:
                    self.root.after(0, lambda: self.copy_response_2input_btn.configure(state=tk.NORMAL))
                ...
                response = self.groq_OSS120B_api(prompt, task_type, model_name, selected_purpose)
                is_error = response.startswith("❌")
                if not is_error:
                    self.root.after(0, lambda: self.copy_response_2input_btn.configure(state=tk.NORMAL))
                    self.root.after(0, lambda: self.notion_upload_btn.configure(state=tk.NORMAL))
    ...
    def claude_api(self, prompt, task_type="request", model_name="Claude Sonnet 4(일반)", purpose_prompt=PURPOSE_PROMPTS["writer"]):
        ...
        try:
            ...
            if model_name == "Claude Opus 4(정교함/비쌈)":
                ...
                model_selected = "claude-opus-4-1-20250805"
                max_tokens_4this = 20000
    ...
    def openai_api(self, prompt, task_type="request", model_name="GPT-4.1(일반)", purpose_prompt=PURPOSE_PROMPTS["writer"]):
        ...
        try:
            ...
            #모델 이름에 따라서 모델을 선택한다.   model_selected = "gpt-5"
            #                model_selected = "gpt-5-mini"
            #                model_selected = "gpt-5-nano"

            # OpenAI API 모델에 따라 다르게 처리

            if model_selected == "gpt-5" or model_selected == "gpt-5-mini" or model_selected == "gpt-5-nano":
            # OpenAI API 호출 (gpt-5 모델은 다르게 처리)
            ...
            # 최종 응답 구성
            full_response = content
            success_message = f"✅ {model_selected} 응답 완료\n\n{full_response}"
            # 클립보드용 응답 저장 (순수 텍스트만)
            self.last_response = full_response
            # 자동으로 클립보드에 복사
            self.auto_copy_to_clipboard(full_response)
            ...
            error_msg = f"❌ {model_selected} API 오류: {str(e)}"
            return error_msg

              
    def groq_OSS120B_api(self, prompt, task_type="request", model_name="OpenAI-OSS-120B", purpose_prompt=PURPOSE_PROMPTS["writer"]):
        try:
            ...
            # 최종 응답 구성
            full_response = content
            success_message = f"✅ gpt-oss-120b 응답 완료\n\n{full_response}"
            # 클립보드용 응답 저장 (순수 텍스트만)
            self.last_response = full_response
            # 자동으로 클립보드에 복사
            self.auto_copy_to_clipboard(full_response)
            ...
            error_msg = f"❌ OpenAI-OSS-120B API 오류: {str(e)}"
            return error_msg
    ...
    def load_user_settings(self):
        ...
        try:
            ...
            def set_if_valid(var, combo, value):
                if value and value in combo['values']:
                    var.set(value)
            
            set_if_valid(self.model1_var, self.model1_combo, settings.get('MODEL1'))
            set_if_valid(self.model2_var, self.model2_combo, settings.get('MODEL2'))
            set_if_valid(self.model3_var, self.model3_combo, settings.get('MODEL3'))
            if settings.get('PURPOSE') in self.purpose_combo['values']:
                self.purpose_var.set(settings.get('PURPOSE'))
        except Exception as e:
            print(f"설정 로드 오류: {e}")

    def save_user_settings(self):
        """사용자 선택을 setting.env에 저장"""
        try:
            data = {
                'MODEL1': self.model1_var.get(),
                'MODEL2': self.model2_var.get(),
                'MODEL3': self.model3_var.get(),
                'PURPOSE': self.purpose_var.get(),
            }
            with open(SETTINGS_FILE, 'w', encoding='utf-8') as f:
                f.write('# Cowrity user settings\n')
